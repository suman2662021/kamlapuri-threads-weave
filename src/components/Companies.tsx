...
const Companies = () => {
  ...
  return (
    <section id="companies" className="py-20 bg-textile-light-blue/30 relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 relative">
        ...
        <div className="text-center mb-16 animate-fade-in">
          ...
          <p className="text-textile-blue/70 text-lg max-w-3xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-8 mb-16">
          {[
            ...
            <div 
              key={index}
              className={`text-center bg-white rounded-xl p-6 shadow-lg border border-textile-light-blue transition-all duration-700 ${
                isVisible ? 'animate-slide-in-left' : 'opacity-0'
              }`}
              style={{ animationDelay: `${index * 0.2}s` }}
            >
              <div className="text-3xl lg:text-4xl font-playfair font-bold text-textile-blue mb-2">
                {stat.number}
              </div>
              <div className="text-textile-blue/70 font-medium">
                {stat.label}
              </div>
            </div>
          ))}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {partners.map((partner, index) => (
            <div
              key={index}
              className={`group bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-500 border border-textile-light-blue hover:border-textile-blue cursor-pointer ${
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <h3 className="font-playfair text-lg font-semibold text-textile-blue group-hover:text-textile-blue-dark transition-colors">
                    {partner.name}
                  </h3>
                  <p className="text-textile-blue font-medium text-sm mt-1">
                    {partner.location}
                  </p>
                </div>
                <div className="w-12 h-12 bg-textile-blue rounded-full flex items-center justify-center text-white font-bold text-sm group-hover:bg-textile-blue-dark transition-colors">
                  {partner.name.split(' ').map(word => word[0]).join('').slice(0, 2)}
                </div>
              </div>
              
              <div className="border-t border-textile-light-blue pt-4">
                <p className="text-textile-blue/70 text-sm">
                  <span className="font-medium">Specialty:</span> {partner.specialty}
                </p>
              </div>

              {/* Hover Effect */}
              <div className="absolute inset-0 bg-gradient-to-r from-textile-blue/5 to-textile-blue/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
            </div>
          ))}
        </div>

        {/* Partnership Benefits */}
        <div className="mt-16 bg-gradient-to-br from-textile-blue to-textile-blue-dark rounded-2xl p-8 lg:p-12 text-center animate-fade-in">
          <h3 className="font-playfair text-2xl lg:text-3xl font-semibold text-white mb-6">
            ...
            {[
              ...
              <div key={index} className="text-center">
                <div className="w-16 h-16 bg-white rounded-full mx-auto mb-4 flex items-center justify-center">
                  <div className="w-6 h-6 bg-textile-blue rounded-full"></div>
                </div>
                <h4 className="font-playfair text-xl font-semibold text-white mb-3">
                  {benefit.title}
                </h4>
